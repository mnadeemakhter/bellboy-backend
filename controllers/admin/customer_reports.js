...
const CustomerReportService = require("../../services/customer_reports");
...
class customerReports {
  ...
  genrateFileForSingleCustomer = async (
    ...
    return new Promise(async (resolve, reject) => {
      ...
      row = 12;
      let column = 0;
      ws.cell(11, 1, 11, 3, true).string("Hiring Detail").style(headerCellStyle);
      keys.forEach((k) => {
        column++;
      ...
      hirings.forEach((h) => {
        ...
        column++;
        ws.cell(row, column).string(h.bellboy && h.bellboy._id.toString() || "").style(bodyCellStyle);
  ...
  getTotalCustomerReport = async (req, res) => {
    try {
      const data = req.query;
      let sortBy = req.query.sortBy || "-created_at";
      var currentStartDate = data.start ? new Date(data.start) : new Date();
      ...
      var currentEndDate = data.end ? new Date(data.end) : new Date();
      ...
      const dbQuery = {
        // created_at: {
        //   $gte: new Date(currentStartDate.toISOString()),
        //   $lte: new Date(currentEndDate.toISOString()),
        // },
      };
      let filename;
      ...
      if (data.status === true || data.status === "true") {
        dbQuery.status = true;
        filename = "total_customer_active.xlsx";
      } else if (data.status === false || data.status === "false") {
        dbQuery.status = false;
        filename = "total_customer_block.xlsx";
      } else {
        filename = "total_customer.xlsx";
      }
      const customers = await CustomerReportService.getCustomersForAdminReport(
        dbQuery,
        sortBy
      );
  ...
  getMonthReport = async (req, res) => {
    try {
      ...
      const customers = await CustomerReportService.getCustomerByMonth(
        year,
        month
      );
      if (!customers) throw new apiErrors.ResourceNotFoundError();
  ...
  getCustomerReportByDevice = async (req, res) => {
    try {
      const data = req.query;
      let sortBy = req.query.sortBy || "-created_at";
      ...
      let filename;
      if (data.device === 1) {
        dbQuery.signup_device = 1;
        filename = `customer_by_device_android.xlsx`;
      } else if (data.device === 2) {
        dbQuery.signup_device = 2;
        filename = `customer_by_device_iOS.xlsx`;
      } else {
        dbQuery.signup_device = 0;
        filename = `customer_by_device_unknown.xlsx`;
      }
      const customers = await CustomerReportService.getCustomersForAdminReport(
        dbQuery,
        sortBy
  ...
  getSingleCustomerReport = async (req, res) => {
    try {
      ...
      const [hirings] = await Promise.all([
        CustomerReportService.getHiringForAdminCustomer({
